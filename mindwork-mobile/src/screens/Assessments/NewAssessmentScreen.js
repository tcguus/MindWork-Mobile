...
export default function NewAssessmentScreen({ navigation, route }) {
  ...
  // Estado para controlar se é edição ou não
  const [isEditing, setIsEditing] = useState(false);
  const [currentId, setCurrentId] = useState(null);

  // --- CORREÇÃO AQUI: RESETAR O ESTADO AO ENTRAR NA TELA ---
  useFocusEffect(
    useCallback(() => {
      ...
      if (assessmentData) {
        ...
        // MODO CRIAÇÃO: Reseta tudo para o padrão
        setIsEditing(false);
        setCurrentId(null);
        setMood(3);
        setStress(3);
      ...
      // Limpa os parâmetros ao sair da tela para evitar bugs futuros
      return () => {
        navigation.setParams({ assessmentData: null });
      };

    }, [route.params?.assessmentData]) // Executa quando os params mudam
  );
  // ---------------------------------------------------------
    ...
    try {
      ...
      if (isEditing && currentId) {
        ...
        Alert.alert('Sucesso', 'Registro atualizado!', [
          { 
            text: 'OK', 
            // CORREÇÃO: Em vez de popToTop, vamos para o Histórico
        ...
        Alert.alert('Sucesso', 'Registro criado!', [
          // Se criar novo, também pode ir pro Histórico ou Home
